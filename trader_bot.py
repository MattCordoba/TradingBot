...
class BinanceBot:
    def __init__(self, api_key, api_secret):
        ...
        self.current_holding_qty = 0.12  # this is in the above
        self.current_holding_value = 0.12  # this is in ETH
    ...
    def document_transaction(self, data_list, filename="c:\\GIT\\mc_dev\\trade_documentation.csv"):
        write_headers = False
        ...
        with open(filename, 'a', newline='') as f:
            writer = csv.writer(f)
            if write_headers:
                writer.writerow(['date', 'holding symbol', 'trade pair', 'type', 'quantity', 'price', 'fee',
    ...
    def trade_buy(self, trade_pair, qty, bid_price=None):
        ...
        print("now holding: %s, qty: %f, value: %f" % (
        self.current_holding, self.current_holding_qty, self.current_holding_value))
...
class VolatilityBot(BinanceBot):
    ...
    def update_values(self, coin="ETH"):
        ...
        for cc in self.coins:
            ...
            for p in current_coin.pairs:
                ...
                try:
                    self.deltas[sym] = (self.current_values[sym] - self.purchase_values[sym]) / self.purchase_values[
                        sym]
                except TypeError:
    ...
    def day_trade(self):
        #order_depth(self, sym, qty, buy_bool=True)
        #self.coins['EOS'].order_depth('ETH',10)
        #return
        ...
        while (self.current_holding_value > self.starting_value * 0.8):
            try:
                ...
                if not current_coin.is_base:
                    ...
                    #get where our spot is in the queue
                    #find the min price we can select s.t. its still > 2x the break_even_delta



                    if waiting.total_seconds() < 20:
                        if self.impatience_level == 0:
                            ...
                            current_ask_price = p
                            current_ask_price_unsantized = current_ask_price
                            current_q = q
                            ...
                            time.sleep(1)
                    elif waiting.total_seconds() > 20 and self.impatience_level == 1:
                    ...
                    #         (q, p) = current_coin.sanitize(current_coin.sym + 'ETH',
                    #                                        qty=self.current_holding_qty,
                    #                                        price=self.purchase_values[
                    #                                                  current_coin.sym + 'ETH'] + 2 * break_even_delta)
                    #         if self.trade_sell(current_coin.sym + 'ETH', q, p):
                    #             self.current_holding = 'ETH'
                    #             continue
                    #     self.impatience_level = 3
                    # # 20-25 min - limit 2x
                    # elif waiting.total_seconds() > 3 * 60 and self.impatience_level == 3:
                    #     print("Growing more impatient")
                    #     # # cancel current order
                    #     # if self.cancel_order(current_coin.sym + 'ETH'):
                    #     #     # place new order at current_price
                    ...
                    #                                        qty=self.current_holding_qty,
                    #                                        price=self.purchase_values[
                    #                                                  current_coin.sym + 'ETH'] + break_even_delta * 1.5)
                    #         if self.trade_sell(current_coin.sym + 'ETH', q, p):
                    #             self.current_holding = 'ETH'
                    #             continue
                    ...
                    if best_trade not in depth_dict:
                        (bid_depth, ask_depth) = current_coin.order_depth(best_trade.replace(current_coin.sym, ""),
                                                                          self.current_holding_qty, True)
                    ...
                    if self.test:
                        ...
                        min_trade_threshold = max(self.minimum_trade_value,
                                                  current_coin.eth_value / current_coin.balance)
                        self.current_holding_qty = min(self.max_trade_value / self.current_values[best_trade],
                                                       current_coin.balance / self.current_values[best_trade])
                    print(
                        "pair:{0:} price:{1:1.6f} delta: {2:1.6f} value increase:{3:1.6f} threshold:{4:1.6f} gap:{5:1.2f}% bid/ask:{6:1.2f}".format(
                            best_trade,
                    ...
                    if abs(self.deltas[best_trade] * self.current_holding_value) > self.minimum_trade_value:
                        ...
                        if current_coin.avg_gap() < 0.5 and depth_dict[best_trade] > 1.5:
                            ...
                            (q, p) = current_coin.sanitize(best_trade, qty=self.current_holding_qty,
                                                           price=self.current_values[best_trade])
                            print()
    ...
    def get_new_sell_price(self,current_coin,qty,price,break_even_delta,queue_limit=10):
        current_coin.update_books()
        ...
        if(spot_in_queue > queue_limit):
            ...
            for a in asks[0:queue_limit]:
                if(a[0] > min_sell_price):
                    sell_price = a[0]
                    break
            return sell_price
        else:
...
class Coin:
    ...
    def update_books(self, symbol=None):
        ...
        if symbol is None:
            ...
            self.gap.appendleft(
                abs(float(self.books[symbol]['bids'][0][0]) - float(self.books[symbol]['asks'][0][0])) / float(
                    self.books[symbol]['asks'][0][0]))
    ...
    def order_depth(self, sym, qty, buy_bool=True):
        ...
        if buy_bool:
            for a in asks[1:]:
                #account for sell walls
                a_qty = a[1]
                #if((100.0 * abs(qty - a_qty )/ qty) > 500): #try to ignore walls
                #    continue
